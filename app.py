...
ANIO_FORECAST = 2026
ZONA_HORARIA_NEGOCIO = ZoneInfo("America/Santiago")


def calcular_periodos(fecha_referencia: datetime | None = None) -> tuple[list[str], list[str]]:
    """Separa meses cerrados y futuros según la fecha local de Chile.

    El mes en curso se mantiene como forecast. Por ejemplo, durante septiembre
    de 2026, enero-agosto se considera histórico y septiembre-diciembre forecast.
    """
    fecha_local = fecha_referencia or datetime.now(ZONA_HORARIA_NEGOCIO)
    if fecha_local.year < ANIO_FORECAST:
        cantidad_historicos = 0
    elif fecha_local.year > ANIO_FORECAST:
        cantidad_historicos = len(MESES)
    else:
        cantidad_historicos = max(0, min(len(MESES), fecha_local.month - 1))

    return MESES[:cantidad_historicos], MESES[cantidad_historicos:]


def rango_meses(meses: list[str]) -> str:
    """Devuelve un rango legible como 'enero a agosto'."""
    if not meses:
        return "sin meses"
    indices = [MESES.index(mes) for mes in meses]
    primero = MESES_LARGOS[indices[0]]
    ultimo = MESES_LARGOS[indices[-1]]
    return primero if primero == ultimo else f"{primero} a {ultimo}"


def rango_meses_corto(meses: list[str]) -> str:
    """Devuelve un rango compacto como 'Ene–Ago'."""
    if not meses:
        return "—"
    return meses[0] if len(meses) == 1 else f"{meses[0]}–{meses[-1]}"


MESES_HIST, MESES_PROY = calcular_periodos()
RANGO_HISTORICO = rango_meses(MESES_HIST)
RANGO_FORECAST = rango_meses(MESES_PROY)
RANGO_HISTORICO_CORTO = rango_meses_corto(MESES_HIST)
RANGO_FORECAST_CORTO = rango_meses_corto(MESES_PROY)
...
st.markdown(
    ...
    <section class="hero">
      <div>
        <p class="eyebrow">Cierre Anual · Ejercicio {ANIO_FORECAST}</p>
        <h1>Forecast Financiero</h1>
        <p>Resultado operacional, márgenes e impacto tributario consolidado en tiempo real.</p>
      </div>
      <div class="phase-track" style="grid-template-columns: {max(len(MESES_HIST), 1)}fr {max(len(MESES_PROY), 1)}fr;">
        <span>Histórico · {RANGO_HISTORICO_CORTO} ({len(MESES_HIST)})</span>
        <span>Forecast · {RANGO_FORECAST_CORTO} ({len(MESES_PROY)})</span>
      </div>
    </section>
    """,
    unsafe_allow_html=True,
...
with grafico_col:
    with st.container(border=True):
        ...
        # Sombra y línea de corte dinámicas para el período de forecast.
        if MESES_PROY:
            inicio_forecast = len(MESES_HIST) - 0.5
            figura.add_vrect(
                x0=inicio_forecast,
                x1=len(MESES) - 0.5,
            ...
            if MESES_HIST:
                figura.add_vline(
                    ...
                    line_dash="dash",
                    line_width=1.5,
                    annotation_text="<b>Inicio Forecast</b>",
        ...
        if MESES_PROY:
            st.caption(
                f"💡 *El área delimitada en azul traslúcido corresponde a la proyección editable "
                f"({RANGO_FORECAST.capitalize()}).*"
            )
        else:
            st.caption("💡 *El ejercicio completo corresponde a información histórica cerrada.*")
