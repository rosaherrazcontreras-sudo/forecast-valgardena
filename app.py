...
# -----------------------------------------------------------------------------
# Configuración dinámica de meses (Barra Lateral)
# -----------------------------------------------------------------------------
...
indice_corte = TODOS_LOS_MESES.index(mes_inicio_forecast)
MESES_HIST = TODOS_LOS_MESES[:indice_corte]
MESES_PROY = TODOS_LOS_MESES[indice_corte:]
...
st.markdown(
    ...
    <section class="hero">
      ...
      <div class="phase-track"><span>{len(MESES_HIST)} meses históricos</span><span>{len(MESES_PROY)} meses forecast</span></div>
    </section>
    """,
    unsafe_allow_html=True,
)
...
with grafico_col:
    with st.container(border=True):
        ...
        figura.add_vline(x=inicio_sombreado, line_color="#aac2df", line_dash="dot", line_width=1)
...
with st.container(border=True):
    ...
    with encabezado_col:
        texto_meses = f"{MESES_PROY[0].lower()} a diciembre" if MESES_PROY else "cerrado"
        st.markdown(
            f"""
            <p class="section-label">Ingreso de proyecciones</p>
            <h2 class="section-title">Forecast · {texto_meses}</h2>
